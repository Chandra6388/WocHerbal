          ...
          <div className="space-y-8 animate-fade-in-up">
            <div className="space-y-6">
              ...
              <div className="relative">
                <h1 className="text-3xl sm:text-4xl md:text-5xl lg:text-7xl font-playfair font-bold text-foreground leading-tight">
                  ...
                  <span className="text-accent block relative group cursor-pointer">
                    <span className="relative z-10 hover:text-muted-foreground transition-colors duration-300">WHY</span>
                    <div className="absolute -inset-4 bg-gradient-to-r from-accent/20 via-muted/20 to-accent/20 blur-xl -z-10 group-hover:blur-2xl transition-all duration-300"></div>
                    <div className="absolute -bottom-2 left-0 w-full h-1 bg-gradient-to-r from-accent to-muted-foreground transform scale-x-0 group-hover:scale-x-100 transition-transform duration-500"></div>
                  </span>
                  <span className="bg-gradient-to-r from-accent via-muted-foreground to-primary bg-clip-text text-transparent hover:from-muted-foreground hover:to-accent transition-all duration-500">WOC</span>
                </h1>

                <div className="text-lg sm:text-xl md:text-2xl lg:text-3xl font-playfair text-muted-foreground mt-4">
                  IS A <span className="text-foreground font-bold bg-gradient-to-r from-foreground via-accent to-muted-foreground bg-clip-text text-transparent animate-pulse">GAME-CHANGER</span>
                  <br />
                  <span className="text-accent font-script text-2xl sm:text-3xl lg:text-4xl block mt-2 hover:text-muted-foreground transition-colors duration-300 cursor-pointer">you'll absolutely love ❤️</span>
                </div>
              </div>
           ...
           <div className="space-y-6">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4">
                {keyIngredients.map((ingredient, index) => <div key={index} className={`bg-gradient-to-br ${ingredient.color} p-3 sm:p-4 lg:p-5 rounded-2xl text-white shadow-lg transform hover:scale-110 hover:rotate-2 transition-all duration-500 cursor-pointer group relative overflow-hidden`} style={{
                animationDelay: `${index * 0.1}s`
              }}>
                    <div className="absolute top-2 right-2 text-xl sm:text-2xl group-hover:animate-bounce">
                      {ingredient.icon}
                    </div>
                    <div className="font-bold text-sm sm:text-base lg:text-lg mb-1 group-hover:text-gray-100 transition-colors">
                      {ingredient.name}
                    </div>
                    <div className="text-xs sm:text-sm opacity-90 group-hover:opacity-100">
                      {ingredient.benefit}
                    </div>
                  ...
                  <div className="aspect-square overflow-hidden rounded-3xl mb-8 bg-gradient-to-br from-accent/5 via-green-500/5 to-blue-500/5 relative group">
                    ...
                    <div className="absolute bottom-4 left-1/2 transform -translate-x-1/2 flex space-x-2">
                      {productImages.map((_, index) => (
                        <div
                          key={index}
                          className={`w-2 h-2 rounded-full transition-all duration-300 ${index === currentImageIndex ? 'bg-accent scale-125' : 'bg-white/50'
                            }`}
                        />
                      ))}
                    </div>
